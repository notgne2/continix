{ buildPackages, cacert, callPackage, closureInfo, coreutils, docker, e2fsprogs
, findutils, go, jq, jshon, lib, moreutils, nix, pigz, referencesByPopularity
, rsync, runCommand, runtimeShell, shadow, skopeo, stdenv
, storeDir ? builtins.storeDir, substituteAll, symlinkJoin, utillinux, vmTools
, writeReferencesToFile, writeScript, writeText, }:
...
rec {
  ...
  pullImage =
    ...
    , imageDigest, sha256, os ? "linux", arch ? buildPackages.go.GOARCH
  ...
  runWithOverlay = { name, fromImage ? null, fromImageName ? null
    ...
    , postUmount ? "" }:
    let
      result = vmTools.runInLinuxVM (runCommand name {
  ...
  buildImage = args@{
    ...
    let
      ...
      result_raw = runCommand "docker-image-${baseName}" {
        ...
        inherit baseJson;
        fromImage = if fromImage == null then
          ""
        else if fromImage ? raw then
          fromImage.raw
        else
          fromImage;
